    ...
    public partial class HalliburtonRibbon
    {
        ...
        private void ReadMail_PP()
        {
            ...
            try
            {
                ...
                log.Info(getCurrentTime() + " inbox " + DateTime.Now.ToString());
                Outlook.Items items = (Outlook.Items)inBox.Items;
                ...
                log.Info("Reading RFQ mail started");
                if (items != null)
                {
                    ...
                    foreach (object eMail in items)
                    {
                        try
                        {
                            ...
                            if (moveMail != null)
                            {
                                ...
                                // log.Info(scount + " Subject " + Subject);
                                //log.Info("Fwd Mail Subject " + fwdMailSubject);
                                //log.Info("Reply Mail Subject " + repMailSubject);
                                // log.Info();
                                decimal price1 = 0;
                                decimal price2 = 0;
                                decimal price3 = 0;
                                ...
                                if (!string.IsNullOrEmpty(Subject) && !string.IsNullOrWhiteSpace(Subject))
                                {
                                    if (Subject.Contains(fwdMailSubject.ToLower()) && Subject.Contains("-TK-".ToLower()) )
                                    {
                                        String html_msg = moveMail.HTMLBody;

                                        log.Info(scount + " Subject " + Subject);
                                        log.Info(scount + " msg html " + html_msg);
                                        ...
                                        log.Info(rcount + " subject contains forward or re mails");
                                        string vendorCode = titleSubject.Substring(titleSubject.LastIndexOf('-') + 1);


                                        /*((Microsoft.Office.Interop.Outlook.MailItem)moveMail).SaveAs(@"C:\Users\Public\Documents\Sample.html", Microsoft.Office.Interop.Outlook.OlSaveAsType.olHTML);
                                        string html = System.IO.File.ReadAllText(@"C:\Users\Public\Documents\Sample.html");
                                        log.Info("Samplehtml created");*/
                                        ...
                                        log.Info("LoadHtml created");
                                        bool firstRow = true;

                                        var isTABLEExist = doc.DocumentNode.Descendants("table").Any();
                                        log.Info(getCurrentTime() + "isTABLEExist" + isTABLEExist);
                                        if (isTABLEExist)
                                        {
                                            log.Info(getCurrentTime() + "Going Inside isTABLEExist //table[@class='MsoNormalTable'] " + isTABLEExist);

                                            ////table[@id='TK_Table']
                                            if (doc.DocumentNode.SelectSingleNode("//table[@class='MsoNormalTable']") != null)
                                            {

                                                log.Info(getCurrentTime() + "Table has value");
                                            }
                                            else
                                            {
                                                ...
                                                log.Info(getCurrentTime() + "Table is null, cant find TK_Table");
                                            }
                                        ...
                                        if (firstRow)
                                        {
                                            log.Info(getCurrentTime() + "dt not created");
                                        }
                                        ...
                                        if (dt != null)
                                        {
                                            if (dt.Rows.Count > 0)
                                            {
                                                // log.Info("dt created" + dt.Rows.Count.ToString());
                                                dtRFQReceived = dt.Copy();
                                                dtRFQReceived.Columns.Add("pricebreakstatus");
                                                dtRFQReceived.AcceptChanges();
                                            }

                                            //   log.Info("dtRFQReceived created" + dtRFQReceived.Rows.Count.ToString());
                                            ...
                                            if (dtRFQReceived.Rows.Count > 0)
                                            {
                                                ...
                                                {
                                                    // MessageBox.Show("18");
                                                    log.Info(getCurrentTime() + "dtRFQReceived.Rows.Count greater than 0");
                                                    log.Info(getCurrentTime() + "Ordered Part Number" + dtRFQReceived.Rows[r]["Part Number"].ToString());
                                                    ...
                                                    else
                                                    {
                                                        ...
                                                        log.Info(getCurrentTime() + "price1" + price1);
                                                    }
                                                    ...
                                                    else
                                                    {
                                                        ...
                                                        log.Info(getCurrentTime() + "price2" + price2);
                                                    }
                                                    ...
                                                    else
                                                    {
                                                        ...
                                                        log.Info(getCurrentTime() + "price4" + price4);
                                                    }
                                                    ...
                                                    else
                                                    {
                                                        ...
                                                        log.Info(getCurrentTime() + "price10" + price10);
                                                    }
                                                    ...
                                                    else
                                                    {
                                                        ...
                                                        log.Info(getCurrentTime() + "price50" + price50);
                                                    }
                                                 ...
                                                 //    vendorQuote = dtRFQReceived.Rows[r]["Quotation#"].ToString();
                                                 //}
                                                 // log.Info("vendorQuote" + vendorQuote);

                                                log.Info(getCurrentTime() + "vendorcode " + vendorCode.ToString());
                                                ...
                                                if (!string.IsNullOrEmpty(vendorCode.ToString()))
                                                {
                                                    ...
                                                    if (noofrecs == 1)
                                                    {
                                                        if (arrFolderPath[0] != "")
                                                        {
                                                            ...
                                                            log.Info("noofrecs1");
                                                            //  log.Info("vendorattachpath " + vendorattachpath);
                                                            //Added by Rajan on 2/8/2020
                                                            if (!String.IsNullOrEmpty(vendorattachpath))
                                                            {
                                                                //MessageBox.Show("22");
                                                                //   log.Info("Goes inside vendorattachpath");
                                                                ReceivedQuotationPath = vendorattachpath + @"\ReceivedQuotation";
                                                                ...
                                                                if (Directory.Exists(vendorattachpath))
                                                                {
                                                                    ...
                                                                    {
                                                                        Directory.CreateDirectory(ReceivedQuotationPath);
                                                                        log.Info(getCurrentTime() + "vendor attachpath exist but receivedquotationpath not exist, so created");
                                                                    }
                                                            ...
                                                            {
                                                                log.Info(getCurrentTime() + "2Vendor folder PATH IS EMPTY");
                                                            }
                                                        }
                                                    ...
                                                    else if (noofrecs > 1)
                                                    {
                                                        for (int a = 0; a < arrFolderPath.Count(); a++)
                                                        {
                                                            if (arrFolderPath[a] != "")
                                                            {
                                                                ...
                                                                // Read All Attachements of RFQ FOLDER
                                                                var attachments = moveMail.Attachments;
                                                                log.Info(getCurrentTime() + "noofrecsgreaterthan1");
                                                                //    log.Info("attachments created");
                                                                //     log.Info("vendorattachpath " + vendorattachpath);
                                                                //Added by Rajan on 2/8/2020
                                                                if (!String.IsNullOrEmpty(vendorattachpath))
                                                                {
                                                    ...
                                                    else if (noofrecs == 0)
                                                    {
                                                        ...
                                                        //  log.Info("Folder not exist for this vendor in Database");
                                                    }
                                                ...
                                                //   log.Info("vendorattachpath created" + vendorattachpath);
                                            }//dtRFQReceived count >0
                                        }
                                        else
                                        {
                                            log.Info(getCurrentTime() + "datable is empty-no records from sample html");
                                        }//dt not null
                        ...
                        catch (System.Exception ex)
                        {
                            ...
                            log.Info(getCurrentTime() + "Error: " + ex.Message);
                        }
                    }//for loop of all inbox items
                    log.Info(getCurrentTime() + "lstMailItem count" + lstMailItem.Count().ToString());
                    ...
                    if (lstMailItem.Count > 0)
                    {
                        foreach (var MailItem in lstMailItem)
                        {
                            ...
                            MailItem.Move(readRFQFolder);
                            log.Info("MAILS MOVED SUCCESSFULLY TO READ RFQ FOLDER");
                        }
                    ...
                    {
                        MessageBox.Show("No Replied RFQ mails in your InBox");
                        log.Info("No Replied RFQ mails in your InBox");
                    }
                    //MessageBox.Show("Total Forward or Reply mails of RFQ : " + rcount);
                    log.Info("Total Forward or Reply mails of RFQ : " + rcount);
                }
                else
                {
                    MessageBox.Show("No Items in inbox");
                    log.Info(getCurrentTime() + "inboxitemsNULL");
                }

                if (sw != null)
                {
                    log.Info(getCurrentTime() + "sw closed ");
                    sw.Close();
                    //  MessageBox.Show("sw closed");
                }
