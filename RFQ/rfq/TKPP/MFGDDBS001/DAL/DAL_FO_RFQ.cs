...
namespace HalliburtonRFQ.DAL
{
    public class DAL_FO_RFQ
    {
        ...
        public DataSet PP_RFQ_Save(DataTable dtHeader, DataTable dtLiner)
        {

            ParameterCollection objParam = new ParameterCollection();
            try
            {
                objParam.Add("@udt_PP_RFQ_Header", dtHeader);
                objParam.Add("@udt_PP_RFQ_Liner", dtLiner);
                return MsSql.RunStoredProcedureDataSet("usp_INS_PP_RFQ", objParam, "PP_RFQ");
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        public string Fetch_ConfigValues(string configkey)
        {

            ParameterCollection objParam = new ParameterCollection();
            try
            {
                objParam.Add("@ConfigKey", configkey);
                string outconfigvalue;
                outconfigvalue = (string)MsSql.RunStoredProcedureScalar("usp_Fetch_ConfigValue", objParam);
                return outconfigvalue;
            }
            catch (Exception ex)
            {
                throw ex;
